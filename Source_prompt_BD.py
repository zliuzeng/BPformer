...
class EncoderDecoder(nn.Module):
    ...
    def __init__(self, encoder, src_embed, d_model1, d_model2):
        ...
        nn.init.kaiming_uniform_(self.W_E, a=math.sqrt(5))
        nn.init.zeros_(self.W_F)
    ...
    def forward(self, src):
        ...
        # src = self.encode(src)
        src = self.encode(src, self.W_A, self.W_B, self.W_C, self.W_D, self.W_E, self.W_F)
...
batch_size = 16
drop_out = 0.5
lr =0.0000005#0.00002#0.00015 # 0.0002
decay = 0.5
result = []
...
for train_index, test_index in kf.split(X):
    ...
    for epoch in range(0, epochs + 1):
        ...
        for i in range(0, int(X_train.shape[0]) // int(batch_size)):
            ...
            # loss=loss + 0.01 * constraint(device, Model.get_mul_prompt())
            loss.backward()
            optimizer.step()
...
def save_xlsx(ts_result):
    runtime_id = 'Source_prompt_BD-{}'.format(time.strftime('%Y-%m-%d %H-%M-%S', time.localtime()))

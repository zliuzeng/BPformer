...
X = np.load('.\data\\BD_X1_three_dimensions.npy')
Y = np.load('.\data\\\BD_Y1.npy')
seed = 121

best_epoch=125  # Hyperparameter: needs to be adjusted
setup_seed(seed)
epochs = 200
batch_size = 16
drop_out = 0.5
...
for train_index, test_index in kf.split(X):
    ...
    for epoch in range(0, epochs + 1):
        ...
        for i in range(0, int(X_train.shape[0]) // int(batch_size)):
            ...
            optimizer.zero_grad()
            outputs,l1= Model(train_data_batch)
            outputs = outputs.cpu()
            loss = F.cross_entropy(outputs, train_label_batch, reduction='mean') + l1
            loss.backward()
            optimizer.step()
        ...
        if epoch % 5 == 0:
            ...
            for i in range(0, int(X_train.shape[0]) // int(batch_size)):
                ...
                outputs ,_= Model(train_data_batch)
        ...
        if epoch == best_epoch:
            torch.save(Model.state_dict(), './Model-2/' + str(kfold_index) + '.pt')
        # 每一次交叉验证后，保存模型，共保存5次 #保存的是最后一个epoch的模型
    # torch.save(Model.state_dict(), './Model-1/' + str(kfold_index) + '.pt')
...
for i in range(num):
    if max_acc < (acc_list[i] + acc_list[i + num] + acc_list[i + 2 * num] + acc_list[i + 3 * num] + acc_list[
        ...
        ts_result[2] = [2,precision_list[i+ num], recall_list[i+ num], acc_list[i+ num], f1_list[i+ num], auc_list[i+ num]]
        ts_result[3] = [3,precision_list[i+ 2 * num], recall_list[i+ 2 * num], acc_list[i+ 2 * num], f1_list[i+ 2 * num], auc_list[i+ 2 * num]]
        ts_result[4] = [4,precision_list[i+ 3 * num], recall_list[i+ 3 * num], acc_list[i+ 3 * num], f1_list[i+ 3 * num], auc_list[i+ 3 * num]]
        ts_result[5] = [5,precision_list[i+ 4 * num], recall_list[i+ 4 * num], acc_list[i+ 4 * num], f1_list[i+ 4 * num], auc_list[i+ 4 * num]]
print('{}-{}-{}-{}-{:.4}-{:.4}-{:.4}-{:.4}-{:.4}\n'.format(max_epoch, lr, batch_size, decay, max_acc, max_precision,
                                                             max_recall, max_f1, max_auc))
# 保存结果
def save_xlsx(ts_result):
    runtime_id = 'Mask_BD-{}'.format(time.strftime('%Y-%m-%d %H-%M-%S', time.localtime()))
